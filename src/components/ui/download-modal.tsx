...
export default function DownloadModal({ isOpen, onClose }: DownloadModalProps) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      setLoading(false);
      onClose();
      // Redirect to download page with email parameter
            ...
            <div className="flex flex-col items-center">
              ...
              <form onSubmit={handleSubmit} className="w-full max-w-sm flex flex-col gap-4">
                ...
                <button
                  type="submit"
                  disabled={loading}
                  className="w-full inline-flex items-center justify-center rounded-2xl bg-gradient-to-r from-primary-blue to-accent-blue py-3 font-bold text-sm text-white transition-all shadow-[0_4px_20px_rgba(10,57,240,0.3)] hover:opacity-95 active:scale-[0.98] disabled:opacity-50"
                >
                  {loading ? (
